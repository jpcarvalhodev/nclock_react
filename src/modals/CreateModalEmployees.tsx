...
export const CreateModalEmployees = <T extends Record<string, any>>({
  ...
  const validateForm = () => {
    ...
    const enrollNumberValid =
      formData.enrollNumber !== undefined && formData.enrollNumber !== null;
  ...
  const handleClose = () => {
    if (
      window.location.pathname.startsWith("/persons/Departments") ||
      window.location.pathname.startsWith("/persons/Groups")
    ) {
      onClose();
    } else {
  ...
  const tabFieldMap: Record<string, string[]> = {
    dadosProfissionais: [
      // Campos relacionados aos dados profissionais
      "departmentId",
      "groupId",
    ],
  };

  const hasErrorsInTab = (tabKey: string): boolean => {
    return tabFieldMap[tabKey]?.some((fieldKey) => !!errors[fieldKey]) ?? false;
  };
          ...
          <Col md={3}>
            ...
            <Form.Group controlId="formEntidadeId" style={{ marginTop: 12 }}>
              ...
              <OverlayTrigger
                placement="left"
                overlay={
                  <Tooltip id="tooltip-modulos">Escolha a sua entidade</Tooltip>
                }
              >
                <Form.Control
                  as="select"
          ...
          <Nav variant="tabs" className="nav-modal">
            ...
            <Nav.Item>
              <Nav.Link eventKey="dadosProfissionais">
                Dados Profissionais{" "}
                {hasErrorsInTab("dadosProfissionais") && (
                  <span
                    style={{
                      color: "white",
                      backgroundColor: "red",
