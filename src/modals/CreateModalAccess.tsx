...
// Define o componente
export const CreateModalAccess = <T extends Record<string, any>>({
  title,
  open,
  onClose,
  onSave,
  ...
  const fetchDropdownOptions = async () => {
    try {
      ...
      const selectedEmployee = sortedEmployees.find(
        (emp) => emp.enrollNumber === initialValues.selectedEmployeeIds
      );
      ...
      if (selectedEmployee && selectedEmployee.employeeCards.length > 0) {
        setSelectedEmployeeCard(
          Number(selectedEmployee.employeeCards[0].cardNumber)
        );
      } else {
        setSelectedEmployeeCard(0);
        if (!hasShownNoCardToastRef.current) {
          toast.warn(
            "O funcionário selecionado não possui cartão associado. Não será possível guardar o acesso."
          );
  ...
  const handleSave = () => {
    const { dropdownData, initialValues, selectedEmployeeIds, ...rest } =
      formData;
    const cleanedData = {
      ...rest,
      pin: selectedEmployeePin,
      cardNo: selectedEmployeeCard,
    };
  ...
  const filteredDoors =
    (dropdownData["eventDoorId"] as Doors[] | undefined)?.filter(
      (door) => door.devSN === formData.deviceSN
    ) ?? [];
        ...
        <Row>
            ...
            <Form.Group controlId="formEventTime">
              ...
              >
                <Form.Control
                  ...
                  value={
                    formData.eventTime
                      ? new Date(formData.eventTime).toISOString().slice(0, 16)
                      : ""
                  }
          ...
          <Col md={6}>
            <Form.Group controlId="formDeviceSN">
              ...
              {errors.deviceSN && (
                <Form.Text className="text-danger">{errors.deviceSN}</Form.Text>
              )}
            </Form.Group>
          </Col>
        </Row>
