...
export const UpdateModalDeptGrp = <T extends Entity>({
  open,
  onClose,
  onUpdate,
  onDuplicate,
  ...
  useEffect(() => {
    const fetchSubdepartments = () => {
      const subDept = dropdownData.departments.filter(
        (dept) => dept.paiId === formData.code
      );
  ...
  const fetchDropdownOptions = async () => {
    try {
      const sortedEmployee = employees.sort(
  ...
  const handleRowSelectedDeptGrp = (state: {
    selectedRows: (Department | Group)[];
  }) => {
