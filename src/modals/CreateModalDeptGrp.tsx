...
// Define o componente
export const CreateModalDeptGrp = <T extends Record<string, any>>({
  open,
  onClose,
  onSave,
  fields,
  ...
  const [selectedRow, setSelectedRow] = useState<
    Department | Group | Employee | null
  >(null);
  ...
  const handleDepartmentClick = (departmentID: string) => {
    const selectedDept = dropdownData.departments.find(
  ...
  const handleGroupClick = (groupID: string) => {
    const selectedGroup = dropdownData.groups.find(
  ...
  const fetchDropdownOptions = async () => {
    try {
      ...
      const nextCode =
        items.reduce(
          (max: number, item: Department) => Math.max(max, item.code),
          0
        ) + 1;
      return nextCode;
    } catch (error) {
      console.error(
        "Erro ao buscar os dados de funcionários e dispositivos.",
        error
      );
      return undefined;
    }
  ...
  const handleDropdownChange = (e: React.ChangeEvent<FormControlElement>) => {
    ...
    const selectedPai = dropdownData.departments.find(
  ...
  return (
    ...
    >
      <Modal.Header closeButton style={{ backgroundColor: "#f2f2f2" }}>
        <Modal.Title>
          {entityType === "department" ? "Criar Departamento" : "Criar Grupo"}
        </Modal.Title>
      </Modal.Header>
          ...
          <Row>
            <Col md={5}>
              ...
              <h5 style={{ marginTop: 20 }}>
                {entityType === "department" ? "Departamentos" : "Grupos"}
              </h5>
