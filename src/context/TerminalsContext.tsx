...
// Cria o contexto
export const TerminalsContext = createContext<DeviceContextType | undefined>(
  undefined
);
...
export const TerminalsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchAllKioskTransaction = async (
    zktecoDeviceID: Devices
  ): Promise<KioskTransaction[]> => {
    try {
      const transactionData = await apiService.fetchAllKioskTransactions(
        zktecoDeviceID
      );
  ...
  const fetchAllKioskTransactionOnDevice = async (
    zktecoDeviceID: Devices
  ): Promise<KioskTransaction[]> => {
    try {
      const transactionData =
        await apiService.fetchAllKioskTransactionsOnDevice(zktecoDeviceID);
      return transactionData;
    } catch (error) {
      console.error("Erro ao buscar transações:", error);
    }
  ...
  const sendAllEmployeesToDevice = async (
    zktecoDeviceID: Devices,
    employeeID?: string[] | null
  ) => {
    try {
      const data = await apiService.sendAllEmployeesToDevice(
        zktecoDeviceID,
        employeeID
      );
  ...
  const saveAllEmployeesOnDeviceToDB = async (
    zktecoDeviceID: Devices,
    employeeID?: string[] | null
  ) => {
    try {
      const data = await apiService.saveAllEmployeesOnDeviceToDB(
        zktecoDeviceID,
        employeeID
      );
  ...
  const saveAllAttendancesEmployeesOnDevice = async (
    zktecoDeviceID: Devices
  ) => {
    try {
      const data = await apiService.saveAllAttendancesEmployeesOnDevice(
        zktecoDeviceID
      );
  ...
  const deleteAllUsersOnDevice = async (
    zktecoDeviceID: Devices,
    employeeID?: string[] | null
  ) => {
    try {
      const data = await apiService.deleteAllUsersOnDevice(
        zktecoDeviceID,
        employeeID
      );
  ...
  const handleAddDevice = async (device: AllDevices) => {
    try {
      ...
      toast.success(
        deviceData.message || "dispositivo adicionado com sucesso!"
      );
    } catch (error) {
  ...
  const handleUpdateDevice = async (device: AllDevices) => {
    try {
      ...
      const updatedDevices = devices.map((d) =>
        d.zktecoDeviceID === updatedDevice.zktecoDeviceID ? updatedDevice : d
      );
      setDevices(updatedDevices);
      toast.success(
        updatedDevice.message || "Atualização realizada com sucesso!"
      );
    } catch (error) {
  ...
  const handleUpdateMBDevice = async (device: MBDevice) => {
    try {
      ...
      const updatedDevices = mbDevices.map((d) =>
  ...
  const fetchAccessControl = async (): Promise<AccessControl[]> => {
    try {
      ...
      const sortedData = data.sort((a: AccessControl, b: AccessControl) =>
  ...
  const handleAddAccessControl = async (
    ...
    try {
      ...
      toast.success(
        data.message || "Controle de acesso adicionado com sucesso!"
      );
    } catch (error) {
  ...
  const handleUpdateAccessControl = async (
    ...
    try {
      ...
      setAccessControl((prevAccessControls) => [...prevAccessControls, data]);
      toast.success(
        data.message || "Controle de acesso atualizado com sucesso!"
      );
    } catch (error) {
  ...
  const handleUpdateDoor = async (door: Doors) => {
    try {
      ...
      if (Array.isArray(data)) {
        const updatedDoors = door.map((d: Doors) =>
          d.id === data.id ? data : d
        );
  ...
  const handleUpdateAux = async (aux: Auxiliaries) => {
    try {
      ...
      if (Array.isArray(data)) {
        const updatedAux = aux.map((d: Auxiliaries) =>
          d.id === data.id ? data : d
        );
        setAux(updatedAux);
      } else {
  ...
  const fetchEventsDeviceNoPagination = async (
    deviceSN?: string[],
    startDate?: string,
    endDate?: string,
    pageNo?: string,
  ...
  const fetchEventsAndTransactionDevice = async (
    startDate?: string,
    endDate?: string,
    pageNo?: string,
    pageSize?: string
