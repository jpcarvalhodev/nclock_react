...
export interface EntityContextType {
  ...
  loginLogsNoPagination: Logs[];
  loginLogs: Logs[];
  setLoginLogs: React.Dispatch<React.SetStateAction<Logs[]>>;
  historyLogsNoPagination: Logs[];
  historyLogs: Logs[];
...
export const EntityProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [loginLogsNoPagination, setLoginLogsNoPagination] = useState<Logs[]>(
    []
  );
  const [historyLogsNoPagination, setHistoryLogsNoPagination] = useState<
    Logs[]
  >([]);
  ...
  // Função para buscar os logs de login sem paginação
  const fetchAllLoginLogsNoPagination = async (
    startDate?: string,
    endDate?: string,
    userIds?: string[],
    pageNo?: string,
    ...
    try {
      ...
      setLoginLogsNoPagination(data.data);
    } catch (error) {
  ...
  // Função para buscar os logs de histórico sem paginação
  const fetchAllHistoryLogsNoPagination = async (
    startDate?: string,
    endDate?: string,
    userIds?: string[],
    pageNo?: string,
    ...
    try {
      ...
      setHistoryLogsNoPagination(data.data);
    } catch (error) {
      console.error("Erro ao buscar os dados de logs:", error);
    }
    return [];
  ...
  return (
    ...
    >
      ...
      <LoadingModal
        show={
          loadingExportBackup || loadingImportBackup || loadingImportEmployees
        }
      />
    </EntityContext.Provider>
  );
