  ...
  useEffect(() => {
    ...
    const filteredByColumnFilters = filteredBySearchText.filter((employee) =>
      Object.keys(filters).every(
        (key) =>
          filters[key] === "" ||
          String(employee[key]).toLowerCase() === filters[key].toLowerCase()
      )
    );

    const sortedFilteredData = filteredByColumnFilters.sort(
        ...
        <div style={{ display: "flex" }}>
          ...
          >
            <CustomOutlineButton
              ...
              icon="bi bi-copy"
              iconSize="0.8em"
              onClick={() => handleDuplicateAndClose(row)}
            />
          </OverlayTrigger>
          <OverlayTrigger
            placement="top"
          ...
          <DeleteModal
            ...
            entityId={
              selectedEmployeeToDelete
                ? selectedEmployeeToDelete.employeeID
                : ""
            }
          />
        )}
