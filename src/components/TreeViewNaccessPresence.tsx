...
interface TreeViewDataNaccessPresenceProps {
...
// Define o componente
export function TreeViewDataNaccessPresence({
  onSelectEmployees,
}: TreeViewDataNaccessPresenceProps) {
    ...
    const buildDepartmentTree = (dept: Department) => ({
      ...
      children: [
        ...
        ...allEmployees
          ...
          .map((emp: Employee) => ({
            id: `dept-${dept.departmentID}-emp-${emp.shortName}` || "Sem ID",
            label: `${emp.enrollNumber} - ${emp.shortName}` || "Sem Nome",
          })),
      ],
    });
