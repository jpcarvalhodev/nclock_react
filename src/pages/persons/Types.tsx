...
export const Types = () => {
  ...
  const [selectedExternalEntityType, setSelectedExternalEntityType] =
    useState<ExternalEntityTypes | null>(null);
  const [
    selectedExternalEntityTypeForDelete,
    setSelectedExternalEntityTypeForDelete,
  ] = useState<any | null>(null);
  const [filters, setFilters] = useState<Filters>({});
  const [initialData, setInitialData] =
    useState<Partial<ExternalEntityTypes> | null>(null);
  const [currentExternalEntityTypeIndex, setCurrentExternalEntityTypeIndex] =
    useState(0);
  ...
  const updateExternalEntityTypes = async (
    externalEntityType: ExternalEntityTypes
  ) => {
  ...
  const handleEditExternalEntity = (
    externalEntityType: ExternalEntityTypes
  ) => {
    setSelectedExternalEntityType(externalEntityType);
    const sortedTypes = dataEE.externalEntityTypes.sort(
      (a, b) => a.order - b.order
    );
    const typeIndex = sortedTypes.findIndex(
      (type) =>
        type.externalEntityTypeID === externalEntityType.externalEntityTypeID
    );
  ...
  const handleSelectedTypeToDelete = () => {
    const typeIds = Array.from(
      new Set(selectedRows.map((type) => type.externalEntityTypeID))
    );
  ...
  const columnNamesMap = externalEntityTypeFields.reduce<
  ...
  const filteredDataTable = dataEE.externalEntityTypes
    .filter(
      (externalEntityType) =>
        Object.keys(filters).every(
          (key) =>
            filters[key] === "" ||
            (externalEntityType[key] != null &&
              String(externalEntityType[key])
                .toLowerCase()
  ...
  const handleNextType = () => {
    ...
    if (currentExternalEntityTypeIndex < sortedTypes.length - 1) {
      ...
      setSelectedExternalEntityType(
        sortedTypes[currentExternalEntityTypeIndex + 1]
      );
    }
  };
  ...
  const getSelectedFields = () => {
    return externalEntityTypeFields.filter((field) =>
    ...
    <div className="main-container">
          ...
          <div className="buttons-container-others">
            ...
            >
              <CustomOutlineButton
                ...
                onClick={refreshExternalEntitiesTypes}
                iconSize="1.1em"
              />
            </OverlayTrigger>
            <OverlayTrigger
      ...
      {selectedExternalEntityType && (
        <UpdateModalCatProfTypes
          ...
          entityType="tipos"
          canMoveNext={
            currentExternalEntityTypeIndex <
            dataEE.externalEntityTypes.length - 1
          }
