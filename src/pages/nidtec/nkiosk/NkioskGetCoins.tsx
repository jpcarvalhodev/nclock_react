...
export const NkioskGetCoins = () => {
  ...
  const [selectedRecolhaMoedeiro, setSelectedRecolhaMoedeiro] =
    useState<any>(null);
  const [startDate, setStartDate] = useState(formatDateToStartOfDay(pastDate));
  const [endDate, setEndDate] = useState(formatDateToEndOfDay(currentDate));
  const [initialData, setInitialData] =
    useState<Partial<RecolhaMoedeiroEContador> | null>(null);
  ...
  const fetchRecolhasToday = async () => {
    ...
    try {
      ...
      console.error(
        "Erro ao buscar os dados de recolha do moedeiro hoje:",
        error
      );
    }
  };
  ...
  const fetchRecolhasForPreviousDay = async () => {
    ...
    try {
      ...
      console.error(
        "Erro ao buscar os dados de recolha do moedeiro ontem:",
        error
      );
    }
  };
  ...
  const fetchRecolhasForNextDay = async () => {
    ...
    try {
      ...
      console.error(
        "Erro ao buscar os dados de recolha do moedeiro amanhã:",
        error
      );
    }
  };
  ...
  const addRecolhaMoedeiro = async (
  ...
  const updateRecolhaMoedeiro = async (
    recolhaMoedeiro: RecolhaMoedeiroEContador
  ) => {
  ...
  useEffect(() => {
    if (selectedDevicesIds.length > 0) {
      const filtered = getCoins.filter((moveCards) =>
  ...
  const handleSelectedRecolhasToDelete = () => {
    const coinsIds = Array.from(
      new Set(selectedRows.map((recolhas) => recolhas.id))
    );
  ...
  const columns: TableColumn<RecolhaMoedeiroEContador>[] =
    recolhaMoedeiroEContadorFields
  ...
  const totalAmount = filteredDataTable.reduce((total, getCoins) => {
    return (
      total +
      (typeof getCoins.valorTotalRecolhido === "number"
        ? getCoins.valorTotalRecolhido
        : parseFloat(getCoins.valorTotal) || 0)
    );
  }, 0);
  ...
  const totalAmountDifference = filteredDataTable.reduce((total, getCoins) => {
    return (
      total +
      (typeof getCoins.diferencaEuros === "number"
        ? getCoins.diferencaEuros
        : parseFloat(getCoins.valorTotal) || 0)
    );
  }, 0);
  ...
  const getCoinsWithNames = filteredDataTable.map(
  ...
  const getSelectedFields = () => {
    return recolhaMoedeiroEContadorFields.filter((field) =>
          ...
          <div className="datatable-container">
            ...
            <div className="datatable-header">
              ...
              <div className="buttons-container-others">
                ...
                >
                  <CustomOutlineButton
                    ...
                    onClick={refreshRecolhaMoedeiro}
                  />
                </OverlayTrigger>
                <OverlayTrigger
                  placement="top"
              ...
              <div className="buttons-container-data-range">
                ...
                >
                  <CustomOutlineButton
                    ...
                    onClick={fetchRecolhasForPreviousDay}
                    iconSize="1.1em"
                  />
                </OverlayTrigger>
                <OverlayTrigger
                ...
                >
                  <CustomOutlineButton
                    ...
                    onClick={fetchRecolhasForNextDay}
                    iconSize="1.1em"
                    disabled={
                      new Date(endDate) >=
              ...
              <div className="date-range-search">
                ...
                >
                  <CustomOutlineButton
                    icon="bi-search"
                    onClick={fetchCoinsBetweenDates}
                    iconSize="1.1em"
                  />
                </OverlayTrigger>
              </div>
            ...
            <div style={{ display: "flex" }}>
              <div style={{ marginLeft: 10, marginRight: 10 }}>
                <strong>Valor Total Online: </strong>Recolhas -{" "}
                {totalAmount.toFixed(2)}€ | Diferença -{" "}
                {totalAmountDifference.toFixed(2)}€
              </div>
            </div>
          </div>
        )}
