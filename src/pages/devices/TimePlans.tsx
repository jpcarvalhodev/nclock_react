...
export const TimePlans = () => {
  ...
  const [selectedTimePlanToDelete, setSelectedTimePlanToDelete] = useState<
    any | null
  >(null);
  const [showAddModal, setShowAddModal] = useState(false);
  const [showUpdateModal, setShowUpdateModal] = useState(false);
  const [selectedTimePlan, setSelectedTimePlan] = useState<TimePlan | null>(
    null
  );
  ...
  const sortedTimePlans = [...timePlans].sort((a, b) =>
    (a.nome || "").localeCompare(b.nome || "")
  );

  // Função para selecionar o próximo plano de horário
  const handleNextTimePlan = () => {
    const currentIndex = sortedTimePlans.findIndex(
      (plan) => plan.nome === selectedTimePlan?.nome
    );
      ...
      <div style={{ display: "flex" }}>
        ...
        >
          <CustomOutlineButton
            ...
            onClick={() => handleEditTimePlan(row)}
          />
        </OverlayTrigger>
        <OverlayTrigger
          placement="top"
  ...
  const getSelectedFields = () => {
    return timePlanFields.filter((field) =>
    ...
    <div className="dashboard-container">
        ...
        <div className="datatable-header">
          ...
          popperConfig={{
            ...
            >
              <CustomOutlineButton
                icon="bi bi-trash-fill"
                onClick={handleSelectedTimePlanToDelete}
                iconSize="1.1em"
              />
            </OverlayTrigger>
            <ExportButton
      ...
      {selectedTimePlan && (
        <UpdateTimePlansModal
          ...
          canMovePrev={
            selectedTimePlan &&
            timePlans.findIndex((plan) => plan.nome === selectedTimePlan.nome) >
              0
          }
          canMoveNext={
            selectedTimePlan &&
